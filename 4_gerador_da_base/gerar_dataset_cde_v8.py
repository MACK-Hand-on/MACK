...
=============================================================================
 ...
 GERADOR v8 - "GEMEO ESTATISTICO" (calibrado + realismo avancado)
=============================================================================
 ...
 v8 corrige tres limitacoes encontradas na EDA da Sprint 2:
 - L1 SAZONALIDADE: o volume diario era plano (197,8 +/- 15,2). Agora tem efeito
   de dia da semana, janela de vencimento de fatura, tendencia de crescimento
   e picos de incidente.
 - L3 FUSO HORARIO: 90% do volume caia entre 8h e 19h de Brasilia, embora 70%
   da base seja CDE (domiciliado no exterior). Agora cada cliente CDE tem um
   fuso, e o horario comercial DELE e convertido para horario de Brasilia.
 - L5 ATRITO ACUMULADO: ser reabertura nao mudava nada no desfecho. Agora
   contatos previos e reabertura reduzem a chance de resolucao pelo bot e
   elevam abandono e sentimento negativo - o ciclo de atrito passa a existir.

 SAIDA: dataset_kenzie360_atendimentos.csv e dataset_kenzie360_mensagens.csv
 RODAR: pip install pandas numpy ; python gerar_dataset_cde_v8.py
=============================================================================
"""
import pandas as pd
import numpy as np
...
_SAIDA = _Path(__file__).resolve().parent.parent / "2_dados" / "atual_v8"
...
CONFIG = {
    ...
    # --- v8 / L5: atrito acumulado -------------------------------------
    "atrito_por_contato": 0.07,   # queda relativa de p_bot por contato previo (teto: 6)
    "atrito_reabertura": 0.18,    # queda adicional quando a conversa e reabertura
    "atrito_piso": 0.45,          # p_bot nunca cai abaixo deste fator
    "abandono_por_contato": 0.10, # aumento relativo da chance de abandono por contato
    "abandono_reabertura": 0.30,  # aumento adicional se for reabertura
    # --- v8 / L1: sazonalidade -----------------------------------------
    "tendencia_periodo": 0.22,    # crescimento do volume do inicio ao fim (22%)
    "n_picos_incidente": 5,       # dias com pico de demanda (falha sistemica)
    "forca_pico": 1.6,            # multiplicador do volume nesses dias
    # --- v8 / L3: fuso horario -----------------------------------------
    "p_horario_comercial": 0.82,  # % das conversas no horario comercial LOCAL do cliente
    # --- v8 / RECALIBRACAO ---------------------------------------------
    # O atrito derruba a resolucao media e eleva o abandono. Como os niveis
    # AGREGADOS (25,3% de resolucao, 17,1% de abandono) vem da calibracao com
    # a operacao real, eles precisam ser preservados: o que a v8 muda e a
    # DISTRIBUICAO interna, nao o total. Estes dois fatores compensam o efeito.
    "calib_p_bot": 1.185,
    "calib_abandono": 0.80,
}
...
# ============ v8 / L3: FUSO HORARIO DO CLIENTE ============================
# O publico CDE mora fora do Brasil. Um gemeo fiel precisa refletir que o
# "horario comercial" do cliente nao e o horario comercial de Brasilia.
# O idioma funciona como proxy razoavel da regiao de domicilio.
# Valores = offset UTC. Brasilia = -3.
FUSO_BR = -3
FUSOS_CDE = {
 # (offsets, pesos) - EN: EUA/Canada, Reino Unido, Europa, Asia
 "EN": ([-5, -8, 0, 1, 9], [0.36, 0.14, 0.24, 0.20, 0.06]),
 # ES: Espanha, Argentina/Uruguai, Colombia/Peru, Mexico
 "ES": ([1, -3, -5, -6], [0.46, 0.20, 0.22, 0.12]),
 # PT: Portugal, brasileiros que voltaram, Africa lusofona
 "PT": ([1, -3, 2], [0.55, 0.35, 0.10]),
}
...
def estilizar(texto,tech,idioma,sent,primeira=False,code_switch=False):
    ...
    return t
def escolher_sentimento(p_bot,est,churn=False,atrito=0.0):
    dif=1-p_bot
    ...
    # v8 / L5: quem ja tentou varias vezes chega mais irritado
    if atrito>0 and est!="fim_ok": pn=min(pn+atrito,0.95)
    r=random.random()
...
def EXPR_POR_SENT(idioma,s):
    ...
    return [""]
def fala_cliente(est,info,idioma,tech,churn=False,code_switch=False,seg="CDE",atrito=0.0):
    s=escolher_sentimento(info["p_bot"],est,churn,atrito)
...
for i in range(1,CONFIG["n_clientes"]+1):
    ...
    # v8 / L3: cliente CDE mora fora - sorteia o fuso a partir do idioma.
    # Nacional e PJ ficam no fuso de Brasilia.
    if seg=="CDE":
        _ops,_ps = FUSOS_CDE[idioma]
        fuso = int(np.random.choice(_ops, p=_ps))
    else:
        fuso = FUSO_BR
...
def simular_conversa(n,cli,categoria,info,inicio,origem,contatos_previos=0,reabertura=False):
    ...
    # ===== v8 / L5: ATRITO ACUMULADO ======================================
    # Na v7 o desfecho dependia so da categoria do assunto. Um cliente na
    # quarta tentativa do mesmo problema era tratado como se fosse a primeira.
    # Aqui o historico passa a pesar em tres frentes: resolucao pelo bot,
    # chance de abandono e humor do cliente.
    _cp = min(contatos_previos, 6)
    fator_bot = 1.0 - CONFIG["atrito_por_contato"]*_cp - (CONFIG["atrito_reabertura"] if reabertura else 0.0)
    fator_bot = max(CONFIG["atrito_piso"], fator_bot)
    fator_abandono = 1.0 + CONFIG["abandono_por_contato"]*_cp + (CONFIG["abandono_reabertura"] if reabertura else 0.0)
    atrito_humor = 0.045*_cp + (0.10 if reabertura else 0.0)
    ...
    def pc(est):  # push cliente: texto coerente com o segmento + eventual PII
        t,s=fala_cliente(est,info,idioma,tech,churn,cs,seg,atrito_humor)
    ...
    def finalizar(status,transf_iniciada,humano_atendeu,resolvida,categoria_f,produto_f,canal_humano,area,motivo,ruido=False,resolvido_bot=False):
        ...
        lc={"id_conversa":f"CONV-{n:06d}","id_cliente":cli["id_cliente"],"segmento_cliente":cli["segmento_cliente"],
            ...
            # v8: agora vem da simulacao (e nao mais do pos-processamento),
            # porque o historico influencia o desfecho desta conversa.
            "contatos_previos":contatos_previos,"reabertura":reabertura}
    ...
    resolvido_bot=random.random()<min(info["p_bot"]*CONFIG["calib_p_bot"]*fator_bot,0.95)   # v8 / L5
    if resolvido_bot:
    ...
    p_ab=min(0.18*ARQUETIPOS[arq]["ab"]*fator_abandono*CONFIG["calib_abandono"],0.55)   # v8 / L5
    roll=random.random()
    if roll<p_ab:
...
# ============ v8 / L1: SAZONALIDADE ======================================
# Na v7 a data era um sorteio uniforme no periodo, o que produzia um volume
# diario plano (197,8 +/- 15,2 - variacao de 7,7%). Nenhuma operacao real se
# comporta assim. Aqui o peso de cada dia combina quatro efeitos.
PESO_DIA_SEMANA = [1.22, 1.10, 1.05, 1.02, 0.95, 0.62, 0.52]  # seg .. dom
N_DIAS = (DATA_FIM - DATA_INICIO).days

def montar_pesos_diarios():
    """Peso relativo de cada dia do periodo."""
    pesos = []
    for i in range(N_DIAS):
        dia = DATA_INICIO + timedelta(days=i)
        w = PESO_DIA_SEMANA[dia.weekday()]
        if 4 <= dia.day <= 10: w *= 1.22   # janela de vencimento de fatura
        if dia.day <= 3:       w *= 1.12   # virada de mes
        w *= (1 + CONFIG["tendencia_periodo"] * i / max(N_DIAS - 1, 1))  # crescimento
        pesos.append(w)
    # picos de incidente (ex.: instabilidade de PIX, falha no app)
    for d in random.sample(range(N_DIAS), CONFIG["n_picos_incidente"]):
        pesos[d] *= CONFIG["forca_pico"]
    tot = sum(pesos)
    return [w / tot for w in pesos]

def sortear_hora(fuso):
    """Hora em BRASILIA, a partir do horario comercial LOCAL do cliente. (v8/L3)"""
    if random.random() < CONFIG["p_horario_comercial"]:
        h_local = random.randint(8, 19)
    else:
        h_local = random.randint(0, 23)
    return (h_local - (fuso - FUSO_BR)) % 24

# ------- loop -------
print(f"Gerando {CONFIG['n_conversas']:,} conversas (gemeo v8)...")
conversas, mensagens = [], []
IDX = np.arange(len(clientes)); CAP = CONFIG["cap_contatos"]
counts = np.ones(len(clientes), dtype=int)   # cada cliente ja tem 1 (cobertura)
PESOS_DIA = montar_pesos_diarios()

# ---- FASE 1: agenda (quem fala, e quando) -------------------------------
# v8: a agenda precisa existir ANTES da simulacao, porque agora o historico
# do cliente influencia o desfecho - e para saber o historico e preciso
# simular em ordem cronologica.
print("  fase 1/2: montando a agenda cronologica...")
agenda = []
for n in range(1, CONFIG["n_conversas"] + 1):
    ...
    d = int(np.random.choice(N_DIAS, p=PESOS_DIA))
    inicio = (DATA_INICIO + timedelta(days=d)).replace(
        hour=sortear_hora(cli["fuso"]),
        minute=random.randint(0, 59),
        second=random.randint(0, 59))
    agenda.append((inicio, cli))

agenda.sort(key=lambda x: x[0])   # ordem cronologica real

# ---- FASE 2: simulacao com memoria por cliente --------------------------
print("  fase 2/2: simulando as conversas...")
historico = {}   # id_cliente -> {"contatos": int, "ultima": str|None}
for n, (inicio, cli) in enumerate(agenda, start=1):
    h = historico.setdefault(cli["id_cliente"], {"contatos": 0, "ultima": None})
    contatos_previos = h["contatos"]
    reabertura = (contatos_previos > 0) and (h["ultima"] in ("Nao", "Desconhecido"))

    cats, pesos = pesos_por_segmento(cli["segmento_cliente"])
    ...
    lc, lm = simular_conversa(n, cli, categoria, info, inicio, origem,
                              contatos_previos, reabertura)
    conversas.append(lc); mensagens.extend(lm)

    # o desfecho de agora e o historico da proxima
    h["contatos"] += 1
    h["ultima"] = lc["resolvida"]

df_conv = pd.DataFrame(conversas)
df_conv = df_conv.sort_values("data_hora").reset_index(drop=True)
df_msg=pd.DataFrame(mensagens)

df_conv.to_csv(_SAIDA / "dataset_kenzie360_atendimentos_v8.csv",index=False,encoding="utf-8-sig",sep=";")
df_msg.to_csv(_SAIDA / "dataset_kenzie360_mensagens_v8.csv",index=False,encoding="utf-8-sig",sep=";")
...
# ================== VERIFICACAO DAS CORRECOES DA v8 =====================
print("\n" + "="*66)
print("  VERIFICACAO DAS CORRECOES v8")
print("="*66)

_d = pd.to_datetime(df_conv['data_hora'])
_dia = _d.dt.date.value_counts().sort_index()
print(f"\n[L1] SAZONALIDADE")
print(f"  volume/dia: media {_dia.mean():.1f} | desvio {_dia.std():.1f} "
      f"| variacao {100*_dia.std()/_dia.mean():.1f}%   (v7 era 7.7%)")
print(f"  min {_dia.min()} | max {_dia.max()}")
_sem = df_conv.groupby(_d.dt.dayofweek).size()
print("  por dia da semana (0=seg):"); print("   ", _sem.to_dict())

print(f"\n[L3] FUSO HORARIO")
_h = _d.dt.hour
_com = ((_h >= 8) & (_h <= 19)).mean()
print(f"  volume entre 8h e 19h de Brasilia: {100*_com:.1f}%   (v7 era 90%)")
_cde = df_conv['segmento_cliente'] == 'CDE'
print(f"    CDE     : {100*((_h[_cde]>=8)&(_h[_cde]<=19)).mean():.1f}%")
print(f"    Nacional: {100*((_h[~_cde]>=8)&(_h[~_cde]<=19)).mean():.1f}%")

print(f"\n[L5] ATRITO ACUMULADO")
_g = df_conv.groupby('reabertura').agg(
    conversas=('id_conversa','size'),
    bot=('resolvido_bot','mean'),
    negativo=('sentimento_geral', lambda s: (s=='Negativo').mean()),
    csat=('csat','mean'))
_g['bot'] = (100*_g['bot']).round(1); _g['negativo'] = (100*_g['negativo']).round(1)
_g['csat'] = _g['csat'].round(2)
print(_g.to_string())
_f = df_conv.groupby(df_conv['contatos_previos'].clip(upper=5))['resolvido_bot'].mean()*100
print("  resolucao do bot por contatos previos (0..5+):")
print("   ", _f.round(1).to_dict(), "  (v7 era ~25.3% em todos)")
